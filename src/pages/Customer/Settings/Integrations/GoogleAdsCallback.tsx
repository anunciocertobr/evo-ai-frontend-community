...
export default function GoogleAdsCallback() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const response = await integrationsService.handleGoogleAdsCallback(code, state);
        setStatus('success');
        setMessage(
          response.email
            ? `Conta ${response.email} conectada! Agora escolha a conta de anúncios.`
            : 'Conta Google conectada! Agora escolha a conta de anúncios.',
        );
        toast.success('Login com Google concluído!');
        setTimeout(() => navigate('/settings/integrations/google_ads'), 1500);
      } catch (err) {
        setStatus('error');
        const errorMessage =
